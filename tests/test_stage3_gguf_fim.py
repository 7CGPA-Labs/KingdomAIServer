...
def test_fim_bracket_deduplication():
    # If the model produces a closing bracket that already exists in the suffix, deduplicate it
    clean = FIMFormatter.clean_completion("return 10; }", suffix="}")
    assert clean == "return 10; "

    clean2 = FIMFormatter.clean_completion("println(\"hi\"); )", suffix=")")
    assert clean2 == "println(\"hi\"); "

def test_in_flight_abort_controller():
    orch = LlamaCppOrchestrator()
    assert orch._active_fim_abort.is_set() is False
    orch.cancel_active_fim()
    assert orch._active_fim_abort.is_set() is True
